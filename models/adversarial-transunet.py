...
class NetDis(torch.nn.Module):
    ...
    def forward(self, sample):
        ...
        x, x1, x2, x3 = self.encoder(sample)

        return torch.cat((sample.flatten(start_dim=1), x.flatten(start_dim=1), x1.flatten(start_dim=1), x2.flatten(start_dim=1), x3.flatten(start_dim=1)), dim=1)
