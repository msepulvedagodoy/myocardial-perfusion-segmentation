...
class TransUnetEncoder(torch.nn.Module):
    ...
    def __init__(self, img_dim, init_features, patch_dim, in_channels, classes, embedding_dim, num_blocks, num_heads, linear_dim, dropout) -> None:
        ...
        self.layer1 = torch.nn.Conv2d(in_channels=in_channels, out_channels=self.features, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.layer5 = ViT(img_dim=img_dim, in_channels=self.features*8, patch_dim=self.patch_dim, classes=classes, embedding_dim=embedding_dim, num_blocks=num_blocks, num_heads=num_heads, linear_dim=linear_dim, dropout=dropout)
...
class TransUnet(torch.nn.Module):
    def __init__(self, img_dim=128, patch_dim=16, embedding_dim=512, init_features=64, in_channels=1, classes=3, num_blocks=6, num_heads=8, linear_dim=1024, dropout=0.1) -> None:
        super(TransUnet, self).__init__()

        self.encoder = TransUnetEncoder(img_dim=img_dim, init_features=init_features, patch_dim=patch_dim, in_channels=in_channels, classes=classes, embedding_dim=embedding_dim,num_blocks=num_blocks, num_heads=num_heads, linear_dim=linear_dim, dropout=dropout)
